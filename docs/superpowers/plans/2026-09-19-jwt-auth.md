...
- **Test budget (deliberate):** 13 tests — 3 in `tests/test_security.py`, 9 in `tests/test_auth.py`, 1 added to `tests/test_health.py` — plus the `test_migrations.py` updates. There are no layer-by-layer unit suites for the repositories or `AuthService`: the HTTP tests cover them. Do not add tests beyond this budget unless asked.
...
| `backend/tests/test_security.py` | The three token/primitive assertions (3 tests). |
| `backend/tests/test_auth.py` | End-to-end HTTP tests incl. the envelope (9 tests). |
...
- Create: `backend/app/db/models/user.py`, `backend/app/db/models/user_session.py`, `backend/app/repositories/base.py`, `backend/app/repositories/user.py`, `backend/app/repositories/session.py`, `backend/app/repositories/__init__.py`, one new file under `backend/migrations/versions/`
...
There are no repository unit tests in this task: per the minimised test budget that layer is
covered end-to-end in Task 5, while `test_migrations.py` and `alembic check` still prove the
schema change.

- [ ] **Step 1: Delete the Item slice and fix every importer**
...
- [ ] **Step 2: Trim `app/schemas/common.py` to just the envelope**
...
- [ ] **Step 6: Apply it and prove models and migrations agree**
...
- [ ] **Step 7: Update `tests/test_migrations.py`**
...
- [ ] **Step 8: Update the OpenAPI assertion in `tests/test_health.py`**
...
- [ ] **Step 9: Remove the Item documentation from `README.md`**
...
Expected: `test_security.py`, `test_health.py` and `test_migrations.py` all pass; `test_items.py`
no longer exists. `alembic check` inside `test_migrations.py` passing is the
...
This task has no test step of its own: `AuthService` is exercised by the end-to-end tests in
Task 5, per the minimised test budget in Global Constraints.
...
- [ ] **Step 4: Re-export the new schemas**
...
```bash
cd /mnt/Data/dubbing && git add backend/app/schemas backend/app/services && git commit -m "feat: add the auth service and its schemas"
```
...
Expected: 9 tests PASS. If the expired case inside `test_me_rejects_every_kind_of_bad_token`
fails, check the `exp` handling in `decode_access_token`.
...
Expected: ruff clean, all 13 tests pass (`test_auth.py`, `test_health.py`,
`test_migrations.py`, `test_security.py`), and `alembic check` reports no pending operations.
...
| §9 Testing | Task 1 (3 primitives), Task 2 (1 envelope test), Task 5 (9 end-to-end + migration updates) |
...
**Test budget.** Deliberately minimised to keep implementation fast: 13 tests — 3 token
primitives (Task 1), 1 envelope-wiring test (Task 2), 9 end-to-end (Task 5) — plus the
migration test updates. The repository layer and `AuthService` have no unit suites of their
own. The gaps this leaves (reuse beyond a single replay, an expired session row, a tampered
payload, a token for a deleted user, the flush-not-commit rule, the production secret guard) are
implemented but unasserted, and are listed as accepted gaps in the spec's §9.
